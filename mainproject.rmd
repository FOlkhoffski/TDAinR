...
Загружаем датасет Imdb, в котором есть часть наших фильмов (в итоге их оказалось 105), чтобы получить жанры

```{r}
data_path = "~/imdb_top_1000.csv"
genres <- read.csv(data_path)
genres = genres %>% select(Series_Title, Genre)
genres <- rename(genres, title = Series_Title)

#избавляемся от лишних знаков в названиях, чтобы понятно всё соотносилось
regex <- "\\(.*\\)"
new_titles <- gsub(regex, "", metadata1$title)
metadata1$title <- gsub("\\(.*\\)", "", new_titles)
metadata1$title <- str_trim(metadata1$title) 

# создадим новый столбец 'genre' в 'metadata1'
metadata1$genre <- NA

...
```{r}
user_ratings_matrix = recc_data_test
get_recommendations <- function(userID, filmsID, ratings) {
  ...
  add_empty_row(user_ratings_matrix)
  colnames(user_ratings_matrix[-1]) <- userID
  for (i in seq_along(colnames(user_ratings_matrix))){
    for (k in seq_along(filmsID)){
      if (conames(user_ratings_matrix[-1,i]) == filmsID[k]) {
        user_ratings_matrix[-1,i] <- ratings[k]
      }
    }
  }

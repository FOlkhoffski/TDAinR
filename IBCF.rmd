...
```
recommend_movies_ibcf <- function(ratings_data, user_id, seed = 100, test_size = 0.2, n_recommendations = 6) {
  set.seed(seed)
  test_ind <- sample(1:nrow(ratings_data), size = nrow(ratings_data) * test_size)
  recc_data_train <- ratings_data[-test_ind, ]
  recc_data_test <- ratings_data[test_ind, ]
  ...
  print(model_details$sim[1:10, 1:10])
  
  # Проверяем, что пользователь в тестовых данных
  if (user_id %in% test_ind) {
    # Делаем предсказания для тестовых данных
    recc_predicted <- predict(object = recc_model, newdata = recc_data_test, n = n_recommendations)
    
    # Возвращаем рекомендации для конкретного пользователя
    return(as(recc_predicted, "list")[[which(test_ind == user_id)]])
  } else {
    stop("Пользователь не найден в тестовой выборке")
  }
}

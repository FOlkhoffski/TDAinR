...
```{r}
get_recommendations <- function(user_ratings) {
  # Пусть user_ratings - это вектор оценок фильмов, поставленных пользователем, где имена элементов - это ID фильмов.

  user_ratings_matrix <- matrix(NA, nrow = 1, ncol = length(movie_ids))
  colnames(user_ratings_matrix) <- movie_ids
  
  # Заполним матрицу оценок пользователя
  user_ratings_matrix[1, names(user_ratings)] <- user_ratings
  
  user_ratings_matrix <- as(user_ratings_matrix, "realRatingMatrix")

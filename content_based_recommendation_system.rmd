...
```{r}
metadata_net = "~/project_personal_files/metadata1_net_new.csv"

metadata <- read.csv(metadata_net)
...
```{r}
contentBasedRecommendation("Skyfall", genre='Comedy', count=10)
```

Сценарий финального отчета:
"Порекомендуйте методом CB что-нибудь пользователю, который высоко оценил самый низкооцененный фильм в данных"

Для начала найдем пользователя, который высоко оценил самый низкооцененный фильм.
Для этого нужно найти фильм с минимальной оценкой.
...
```{r}
ratings %>% filter(user_id == 131595) %>% inner_join(metadata, by = 'item_id')
```
